...
class ProductionModelTrainer:
    ...
    def run_full_pipeline(self, model_types=['logistic_regression'], n_trials=50):
        ...
        best_model_type = max(best_models.keys(), 
                             key=lambda k: best_models[k]['val_f1'])
        ...
        model_info = self.save_final_model(
            final_model, best_models[best_model_type], 
            test_metrics, label_encoder, stats
        )
    ...
    def save_final_model(self, model, model_info, test_metrics, label_encoder, dataset_stats):
