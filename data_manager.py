...
class DataManager:
    ...
    def load_huggingface_dataset(self, force_reload=False):
        ...
        try:
            ...
            train_df, val_df = train_test_split(
                ...
                random_state=42,
                stratify=(
                    train_df["sentiment"] if "sentiment" in train_df.columns else None
                ),
            )
    ...
    def preprocess_text(self, df, text_column="review"):
        ...
        if removed_count > 0:
            self.logger.warning(
                f"Removed {removed_count} empty reviews after preprocessing"
            )
    ...
    def encode_labels(self, train_df, val_df, test_df):
        ...
        train_df["sentiment_encoded"] = label_encoder.fit_transform(
            train_df["sentiment"]
        )
    ...
    def get_data_statistics(self, train_df, val_df, test_df):
        ...
        stats = {
            ...
            "val_sentiment_dist": {
                str(k): int(v)
                for k, v in val_df["sentiment"].value_counts().to_dict().items()
            },
            "test_sentiment_dist": {
                str(k): int(v)
